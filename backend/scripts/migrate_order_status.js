...
async function migrateStatus() {
    ...
    try {
        console.log('🚀 Starting Order status migration: in_transit -> in_transit');

        // Update Orders
        const [orderCount] = await Order.update(
            { status: 'in_transit' },
            { where: { status: 'in_transit' }, transaction }
        );
        console.log(`✅ Updated ${orderCount} Orders.`);

        // Note: DeliveryTask status 'in_progress' is usually what corresponds to 'in_transit'.
        // We don't necessarily need to rename DeliveryTask.status unless it was literally 'in_transit'.
