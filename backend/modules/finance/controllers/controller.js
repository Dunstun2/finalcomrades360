const { Order, PlatformConfig, sequelize, Transaction, Wallet, User, DeliveryCharge, Notification } = require('../../../database/models.registry');
const { Op } = require('sequelize');
const { moveToPaid } = require('../../../utils/walletHelpers');
const { DeliveryTask, OrderItem, Product, FastFood, Warehouse, PickupStation } = require('../../../database/models.registry');
...
const getSystemIncome = async (req, res) => {
    try {
        const { DeliveryTask, OrderItem, Product, Order } = require('../../../database/models.registry');
            ...
            if (tx && tx.status === 'success') {
                ...
                if (task && !task.systemRevenueClaimed) {
                    ...
                    if (revenue > 0) {
                        ...
                        const { PlatformWallet, PlatformTransaction } = require('../../../database/models.registry');
