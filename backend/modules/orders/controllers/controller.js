...
const { Product, User, Wallet, Transaction, Order, OrderItem, Commission, DeliveryAgentProfile, Cart, FastFood, Service, DeliveryTask, DeliveryCharge, Warehouse, PickupStation, FastFoodPickupPoint, PlatformConfig, Notification, HandoverCode, Batch, Payment, Otp } = require('../../../database/models.registry');

const { calculateCommission: createCommissionRecords } = require('../../finance/controllers/commission.controller');
const { isValidTransition, getValidTransitionsForOrder, autoCreateDeliveryTask } = require('./transition.controller');
...
const createOrderFromCart = async (req, res) => {
  ...
  try {
    ...
    if (promoCode) {
      const { PromoCode } = require('../../../database/models.registry');
    ...
    if (paymentId) {
      const { Payment } = require('../../../database/models.registry');
...
const updateOrderStatus = async (req, res) => {
  ...
  if (['at_warehouse', 'at_warehouse', 'at_pick_station', 'ready_for_pickup'].includes(status)) {
    ...
    const { DeliveryTask } = require('../../../database/models.registry');
...
const unassignDeliveryAgent = async (req, res) => {
  ...
  try {
    ...
    const { DeliveryTask, Op } = require('../../../database/models.registry');
...
const sellerConfirmOrder = async (req, res) => {
  try {
    ...
    if (['awaiting_delivery_assignment', 'seller_confirmed'].includes(updates.status)) {
      try {
        const { autoCreateDeliveryTask } = require('./transition.controller');
...
const editOrder = async (req, res) => {
  ...
  try {
    ...
    // Notify Seller
    try {
      const { User, Product, FastFood } = require('../../../database/models.registry');
