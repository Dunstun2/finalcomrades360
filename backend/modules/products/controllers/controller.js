...
const createProduct = async (req, res, next) => {
  ...
  try {
    ...
    if (existingProduct) {
      if (existingProduct.status === 'draft') {
        ...
        await existingProduct.update({
          ...
          // FIX: Update status and reviewStatus when submitting a previously saved draft!
          status: (!isDraft && (String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'superadmin' || String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'admin')) ? 'active' : (isDraft ? 'draft' : 'pending'),
          ...
          reviewStatus: ((String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'superadmin' || String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'admin') ? 'approved' : 'pending')
        });
    ...
    const product = await Product.create({
      ...
      // Save tags field
      tags: mergedTags,
      // Status management: super_admin/admin products are active immediately
      status: (!isDraft && (String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'superadmin' || String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'admin')) ? 'active' : (isDraft ? 'draft' : 'pending'),
      approved: (!isDraft && (String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'superadmin' || String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'admin')),
      reviewStatus: ((String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'superadmin' || String(req.user?.role || '').toLowerCase().replace(/[^a-z0-9]/g, '') === 'admin') ? 'approved' : 'pending')
    });
...
const updateProduct = async (req, res, next) => {
  ...
  try {
    ...
    if (isDraft) {
      ...
      if (wasAlreadyLive) {
        if (hasPriceChanged) {
          // If they changed the price, pull the item off the live storefront
          // and flag it as pending re-review in the admin queue
