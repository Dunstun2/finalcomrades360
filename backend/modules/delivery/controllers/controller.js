    ...
    if (task.order) {
      ...
      if (status === 'completed') {
        ...
        if (hubTypes.includes(task.deliveryType)) {
          ...
          const t = await sequelize.transaction();
          try {
            const { Commission } = require('../../../database/models.registry');
            ...
            const { PlatformWallet, PlatformTransaction, OrderItem, Product, FastFood, Service } = require('../../../database/models.registry');
...
const getAvailableAgentsForOrder = async (req, res) => {
  try {
    ...
    if (order.deliveryMethod === 'pick_station' && order.pickStation) {
      try {
        const { PickupStation } = require('../../../database/models.registry');
        const { Op } = require('sequelize');
