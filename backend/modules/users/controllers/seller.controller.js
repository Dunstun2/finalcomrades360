...
const { Product, Category, Order, OrderItem, User, FastFood, DeliveryTask, Warehouse, PickupStation, Batch, sequelize } = require('../../../database/models.registry');
...
const updateMyProduct = async (req, res, next) => {
  try {
    ...
    const wasAlreadyLive = row.approved === true && row.status === 'active' && row.reviewStatus === 'approved';
    const hasPriceChanged = updates.basePrice !== undefined && parseFloat(updates.basePrice) !== parseFloat(row.basePrice);
    ...
    if (isDraft) {
      ...
      // Seller submitting final edits or updating live product
      if (wasAlreadyLive) {
        if (hasPriceChanged) {
          // If price changed, pull offline
