...
const googleAuth = async (req, res, next) => {
  ...
  try {
    ...
    if (user) {
      ...
      console.log('[googleAuth] Generating JWT for user ID:', user.id);
      ...
      setImmediate(async () => {
        try {
          await LoginHistory.create({
            userId: newUser.id, ipAddress, browser: 'GoogleRegistration', os: 'Unknown', device: 'Unknown', location: 'Unknown', status: 'success'
          });
        } catch (err) {
          console.error('[authController] Google registration history failed:', err);
        }
      });
