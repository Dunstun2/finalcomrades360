...
import SystemFeedbackModal from '@/shared/components/SystemFeedbackModal'
...
export default function RegisterForm({ onSuccess, initialReferralCode, isModal = false }) {
    ...
    // Keep a ref of the latest form data to prevent stale closures in socket events
    const formDataRef = useRef({ password, referralCode })
    useEffect(() => {
        formDataRef.current = { password, referralCode }
    ...
    const handleVerifyDirect = async (otpString) => {
        ...
        try {
            ...
            const payload = {
                ...
                otp: otpString,
                ...(currentData.referralCode ? { referralCode: currentData.referralCode } : {})
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { joinOtpRoom } = await import('@/shared/services/socket');
            joinOtpRoom(contact);
